...
public class AdminServices {
	...
	private static void filterCustomers(Scanner s) {
		try {
			ArrayList<com.ecommerce.models.Customer> customers = AdminDAO.viewCustomers();
			if(customers.isEmpty()) {
				System.out.println("No customers found.");
				return;
			}
			System.out.println("Filter Options: \n1.By Status(active/inactive).\n2.By name ascending\n3.By name descending\n4.Back");
			int filterChoice = InputValidate.ChoiceValidation(s,1,4);
			switch(filterChoice) {
			case 1:
				String status = InputValidate.StringValidation(s, "Enter status(active/inactive): ").toLowerCase();
				customers.stream()
				.filter(c -> c.getStatus().equalsIgnoreCase(status))
				.forEach(c -> System.out.println("Name: "+c.getName()+" | Email: "+c.getEmail()+" | Status: "+c.getStatus() ));
				break;
	...
	public static void manageCategory(Scanner s) throws Exception{
		...
		do {
			...
			case 1:
				CategoryDAO.viewCategories().stream()
				.forEach(c -> 
			...
			case 2:
				...
				CategoryDAO.addCategory(category_name, desc);
			...
			case 3:
				...
				CategoryDAO.deleteCategory(c_id);
			...
			case 4:
				...
				do {
					...
					case 1:
						...
						CategoryDAO.modifyCategoryName(cat_name, cat_id);
					...
					case 2:
						...
						CategoryDAO.modifyCategoryDesc(cat_desc, cat_id);
					...
					case 3:
						...
						CategoryDAO.modifyCategoryStatus(cat_status, cat_id);
	...
	public static void manageProducts(Scanner s) throws Exception{
		...
		do {
			...
			case 1:
				...
				ProductDAO.addProduct(cat_id, name, brand, desc, price, url);
				System.out.println("New Product added");
				System.out.println();
				break;
				
			case 2:
				ProductDAO.viewProducts().stream()
				.forEach(p -> 
			...
			case 3:
				...
				ProductDAO.modifyProductPrice(pid,newPrice);
	    ...
	    do {
	        ...
	        switch (choice) {
	            ...
	            case 2:
	                ...
	                String status = InputValidate.StringValidation(s, "Enter status (open/in_progress/resolved/closed): ").toLowerCase();
	                while (!status.matches("open|in_progress|resolved|closed")) {
	                    ...
	                    status = InputValidate.StringValidation(s, "Enter status: ").toLowerCase();
	                }
	...
	public static void managePayments() {
		try {
			List<Payment> payments = AdminDAO.viewPayments();
			if(payments.isEmpty()) {
