...
public class PatientService {
    ...
    public boolean isDoctorAvailable(int doctorId, String date, String time) {
        ...
        Doctor doctor = doctorDAO.findByUserId(doctorId);
        if (doctor == null || !"AVAILABLE".equals(doctor.getAvailability())) {
            return false;
        }
        
        // Check if doctor already has an appointment at the same date/time
        List<Appointment> existingAppointments = appointmentDAO.findByDoctorAndDateTime(
            doctorId, 
            Date.valueOf(date), 
    ...
    /**
     * Book appointment with doctor availability check
     */
    public boolean bookAppointment(int patientId, int doctorId, String date, 
        ...
        if (!isDoctorAvailable(doctorId, date, time)) {
            ...
            System.out.println("Please choose another date/time or doctor.");
            return false;
        }
	...
	public int getSlotBookingCount(int doctorId, String date, String time) {
